//
// Created by Falling_jasmine on 2025/8/4.
//
...
int MPU6050_DMP_init(void)
{
    ...
    //启用下面这段代码的话，mpu6050必须水平放置，不然过不了自检
    // ret = run_self_test();
    // if(ret != 0)
    // {
...
int MPU6050_DMP_Get_Date(float *pitch, float *roll, float *yaw)
{
    ...
    if(sensors & INV_WXYZ_QUAT)
    {
        ...
        *pitch = -1.0*(asin(2 * q1 * q3 - 2 * q0 * q2) * 57.3); // pitch
...
/**
 * @brief  配置MPU6050的DMP中断功能
 * @param  enable: 中断使能标志 (1=使能, 0=禁用)
 * @param  trig_edge: 中断触发方式 (1=边沿触发, 0=电平触发)
 * @param  high_level: 触发极性 (1=高电平/上升沿, 0=低电平/下降沿)
 * @retval 0=成功, 非0=失败
 */
